from run_Roberta_model import model_train_validate_test,model_load_test
...
#     test_prediction_dir = 'E:/桌面/sentence_pair_modeling/BQ Corpus/output/Roberta/'
...
if __name__== '__main__':
    ...
    data = pd.concat([train_df]).reset_index(drop=True)

    target_dir = os.path.join(lcqmc_path, "output/Roberta") # load pretrained model
    test_prediction_dir = os.path.join(lcqmc_path, "output/Roberta") # where to save the infer result
